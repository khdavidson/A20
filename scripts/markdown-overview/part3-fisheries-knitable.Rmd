...
- All years map: 
https://github.com/khdavidson/A20/blob/main/outputs/figures/SJ%20CWT%20recoveries%20coastwide%20-%20all%20years.pdf
...
plotCWTrcvy <- function(year_start, year_end, plot_title){
  ...
  geom_jitter(data=CWTrcvySummaryAllyrs %>% 
              ...
              aes(x=long, y=lat, colour=`Fishery PSC Code english`, fill=`Fishery PSC Code english`, 
                  size=overallMeanFinescaleExp_gearPSC, shape=`Fishery Series`), alpha=0.3, width=0.2, height=0.2) +
...
ggplot() +
  ...
  geom_jitter(data=a20cwtRcvy.spatial%>%
              ...
              aes(x=long, y=lat, colour=`Fishery PSC Code english`, fill=`Fishery PSC Code english`, 
                  size=finescaleExp, shape=`Fishery Series`), alpha=0.6, width=0.2, height=0.2) +
  scale_size(range=c(1,5)) +
  scale_shape_manual(values=c(21,22,23,24,25)) +
