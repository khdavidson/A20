...
# Fence install dates -------------------------- 
sj.fence.dates <- read_excel("//dcbcpbsna01a.ENT.dfo-mpo.ca/SCD_Stad/WCVI/ESCAPEMENT/Data/run timing tables (updated).xlsx", sheet="San Juan", range="A4:AE65") %>% 
  rename(year=`...1`,
         system=`81`) %>%
  fill(year) %>%
...
The fence operated by 4 Mile hatchery primarily functions to collect broodstock, and secondarily to count all species. Recent install dates are typically from Labour Day long weekend to Thanksgiving long weekend, as flow allows. It is typically removed just in time for the fall/winter floods (see \@ref(fig:fenceflowfig)), typically at approximately 
