...
crestBDWR.catEst.sj <- left_join(crestBDWR %>% 
  ...
  mutate(estKeptByID = AreaPropnMonthlySamples*Est_Kept,
         seKeptByID = AreaPropnMonthlySamples*SE_Kept)  
    

# Summarize average catch by stock ID of San Juan only fish ------------------
avg_catch_area <- crestBDWR.catEst.sj %>%
  arrange(YEAR, AREA, MONTH) %>%
  filter(isSJ2!="Other") %>% 
  group_by(MONTH, AREA, isSJ2) %>%
...
crestBDWR.catEst.sj$AREA <- factor(crestBDWR.catEst.sj$AREA, levels=c("127", "27", "126", "26", "125", "25", "124", "24", "123", "23", "121", "22", "21", "20", ordered=T))
crestBDWR.catEst.sj$MONTH <- factor(crestBDWR.catEst.sj$MONTH, levels=month.name)
crestBDWR.catEst.sj$MONTH <- factor(crestBDWR.catEst.sj$MONTH, levels=month.name)


ggplot() +
  geom_boxplot(data=crestBDWR.catEst.sj%>%filter(isSJ2!="Other"),
...
# # Join to spatial
 # crestBDWR.catEst.sj.spatial <- sp::merge(x=pfmaPOLY, 
 #                                   y=crestBDWR.catEst.sj %>%
