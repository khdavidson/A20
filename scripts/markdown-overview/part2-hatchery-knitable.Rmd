...
SJotoRef <- full_join(
  ...
  mutate(TM_application_flag = case_when(`INTEND HATCH CODE`!=`ACT ASSIGN HATCH CODE` ~ "mark application issue: actual mark applied was not the intended mark applied",
                                     is.na(`INTEND HATCH CODE`) | is.na(`ACT ASSIGN HATCH CODE`) ~ "unknown",
                                     TRUE ~ "ok")) %>%
  # left_join(.,
  #           readxl::read_excel(
  #             "//dcbcpbsna01a.ENT.dfo-mpo.ca/SCD_Stad/Spec_Projects/Thermal_Mark_Project/Marks/All CN Marks from NPAFC Otolith Database to May 1, 2023.xlsx", 
...
SJ_mark_history_BYrollup <- full_join(
  ...
  SJRelSEP %>% 
    ...
    group_by(BROOD_YEAR) %>%
    summarize(`BY total released` = sum(TotalRelease,na.rm=T),
              `CWT` = round(sum(TaggedNum,na.rm=T)/`BY total released`,3),
              `AD-only` = round((sum(NoTagNum,na.rm=T)+sum(ShedTagNum,na.rm=T))/`BY total released`,3),
              `mark rate` = round((sum(NoTagNum,na.rm=T)+sum(ShedTagNum,na.rm=T)+sum(TaggedNum,na.rm=T))/`BY total released`,3),
              `unmark/untag` = round(sum(UnmarkedNum,na.rm=T)/`BY total released`,3)) %>% 
    rename(`BROOD YEAR`=BROOD_YEAR) 
) %>% 
  mutate(total_rel_check = total_oto_release==`BY total released`) %>%
  print()


#### HERE NEXT DAY: ADD PBT COLUMNS!!!!! 
...
SJ_mark_history_BYrollup %>%
  ...
  #**** NEXT DAY: CLEAN UP COLUMN NAME HEADERS 
  arrange(`BROOD YEAR`) %>%
  kbl(align="c", caption="San Juan chinook otolith marking, CWTing, and adipose clipping over time.") %>%
...
Using recent-year data since 2012 and considering otolith thermal marks, CWTs and adipose clips, the % hatchery-origin in broodstock (pHOB) biosamples has varied considerably from >80% to <5%. 
