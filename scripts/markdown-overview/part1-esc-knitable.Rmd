...
# 2. Extract years published to NuSEDS ------------------------
nuseds_SJ_yrs <- runNuSEDSQuery(query_doc=here("scripts", "json", "nuseds_esc_query_A20.json"), 
...
sj.esc <- full_join(
  ...
  runNuSEDSQuery(query_doc=here("scripts", "json", "nuseds_esc_query_A20.json"), 
                 config_file=here("saaWeb.config")) %>%
    filter(!grepl("GEORGIA STRAIT", `Conservation Unit Name`),
           `Waterbody Name` %notin% c("DE MAMIEL CREEK", "ROCKY CREEK", "AYUM CREEK", "SOOKE RIVER", "MAIDENHAIR CREEK", 
                                      "MUIR CREEK", "FALLS CREEK", "UGLOW CREEK"), 
...
# source(here("scripts", "functions", "pullSanJuanAgeData.R"))
  # saves as SJ_allAgesMaster


# Option 2: Load pre-dumped age data ------------------
SJ_allAgesMaster <- readxl::read_excel(here("outputs", "R_OUT - SanJuan_Ages_allSpp-allYrs 1990-2023.xlsx"),
                                       sheet="Sheet1") %>% 
...
-   Long-term age structure (entire time series): `r sj_LT_avgs %>% filter(Species=="Chinook") %>% arrange(desc(avg_propn)) %>% slice(1) %>% pull(res_age[1])` year olds (`r sj_LT_avgs %>% arrange(desc(avg_propn)) %>% slice(1) %>% pull(avg_propn[1])*100` +/- `r sj_LT_avgs %>% filter(Species=="Chinook") %>% arrange(desc(avg_propn)) %>% slice(1) %>% pull(sd_propn[1])*100`%) followed by `r sj_LT_avgs %>% filter(Species=="Chinook") %>% arrange(desc(avg_propn)) %>% slice(2) %>% pull(res_age[1])` year olds (`r sj_LT_avgs %>% filter(Species=="Chinook") %>% arrange(desc(avg_propn)) %>% slice(2) %>% pull(avg_propn[1])*100` +/- `r sj_LT_avgs %>% filter(Species=="Chinook") %>% arrange(desc(avg_propn)) %>% slice(2) %>% pull(sd_propn[1])*100`%)
...
ggplot() +
  ...
  geom_hline(data=RPs%>%filter(RP=="SMSY"), aes(yintercept=Value), size=1, linetype="dashed") +
    geom_hline(aes(yintercept=10000), size=1, linetype="dashed", colour="gray50") +
  geom_hline(data=sj.esc %>% filter(Species=="Chinook", year>=(max(analysis_year-10)), est_type=="Natural Adult Spawners") %>% 
  ...
  geom_bar(data=sj.esc %>% filter(Species=="Chinook", source=="NuSEDS",
           ...
           stat="identity", alpha=0.8, width=0.9, size=0.3) +
...
pdf(file = here("outputs", "figures", "San Juan chinook age composition 1990-present.pdf"),   
...
ggplot() +
  ...
  geom_text(data=sj.FULL.age %>% 
            ...
            aes(x=`Sample Year`, y=1.07, label=paste0("n=",sample_size)), size=3, angle=45, vjust=1, hjust=0.5) +
...
```{r}
pdf(file = here("outputs", "figures", "San Juan chinook age composition 2012-present.pdf"),   
    width = 11, # The width of the plot in inches
    height = 8.5) # The height of the plot in inches
...
ggplot() +
  ...
  labs(x="Brood year", y="% of annual biosamples", fill="Age:", colour="Age:") +
