...
WCVIcrestBDWR <- read.csv(file=list.files(path="//dcbcpbsna01a.ENT.dfo-mpo.ca/SCD_Stad/WCVI/CHINOOK/WCVI_TERMINAL_RUN/Annual_data_summaries_for_RunRecons/CRESTcompile_base-files/2-Export-from-R/",
  ...
  mutate(yday = lubridate::yday(COLLECTION_DATE),
         ...
         isSJ2 = case_when(grepl("Non-San Juan", isSJ1) ~ "Other",
                           TRUE ~ isSJ1),
         isSJ3 = case_when(isSJ2=="Other" ~ isSJ2,
...
WCVIcrestBDWR.catEst.sj <- left_join(WCVIcrestBDWR %>% 
                                       ...
                                       group_by(YEAR, MONTH, AREA, isSJ2, isSJ3) %>% 
                                       summarize(n3 = n())  %>%
                                       group_by(YEAR, MONTH, AREA, isSJ2) %>%
                                       mutate(n2 = n(),
                                              AreaMonthSampleTotal2 = sum(n2),
                                              AreaPropnMonthlySamples2 = n2/AreaMonthSampleTotal2,
                                              AreaMonthSampleTotal3 = sum(n3),
                                              AreaPropnMonthlySamples3 = n3/AreaMonthSampleTotal3),
  ...
  mutate(estKeptByID3 = AreaPropnMonthlySamples3*Est_Kept,
         seKeptByID3 = AreaPropnMonthlySamples3*SE_Kept,
         estKeptByID2 = AreaPropnMonthlySamples2*Est_Kept,
...
avg_catch_area <- WCVIcrestBDWR.catEst.sj %>%
  ...
  summarize(meanLC = mean(estKeptByID,na.rm=T),
            sdLC = sd(estKeptByID,na.rm=T),
            meanpropn=mean(AreaPropnMonthlySamples,na.rm=T),
            totaln=sum(n,na.rm=T),
...
wcviPFMA_sjCatEst <- left_join(pfma_sf ,
                               WCVIcrestBDWR.catEst.sj %>%
                                 ...
                                 summarize(n=sum(n),
                                           strata_n = sum(AreaMonthSampleTotal,na.rm=T),
                                           propn=n/strata_n,
                                           Est_Kept = unique(Est_Kept),
                                           estKeptByID=propn*Est_Kept) %>%
...
WCVIcrestBDWR.catEst.sj %>%
  filter(isSJ2!="Other") %>%
  select(YEAR, MONTH, AREA, AreaPropnMonthlySamples)
...
ggplotly(
  ggplot(data = WCVIcrestBDWR.catEst.sj %>% 
           mutate(inverse_estKeptByID = case_when(isSJ3=="Other" ~ Est_Kept-estKeptByID,
                                                  TRUE ~ estKeptByID)) %>% 
           mutate(inverse_estKeptByID = case_when(inverse_estKeptByID==0.0000000 ~ NA,
                                                  TRUE ~ inverse_estKeptByID)) %>%
           filter(!is.na(YEAR))) +
    geom_point(aes(y=AREA, x=MONTH, fill=inverse_estKeptByID, colour=inverse_estKeptByID, text=paste0("n=",n)), 
               shape=22, size=3, alpha=0.4) +
  ...
  tooltip = c("AREA", "MONTH", "estKeptByID", "text")
)
```
...
ggplotly(
  ggplot() +
    geom_sf(data=wcviPFMA_sjCatEst%>%st_set_crs(.,"WGS84")%>%filter(!is.na(MONTH) & YEAR=="2022"), 
...
area20REGION.BDWR <- crestBDWR %>% 
  filter(AREA %in% c("20"), SAMPLE_TYPE=="Sport", SPECIES==124, DISPOSITION=="Kept", LEGAL_STATUS=="LEGAL", `(R) RESOLVED ORIGIN-REGION ID`%notin%c("Unknown NA", "Unknown Unknown (<75% GSI assignment)")) %>% 
...
# FOCUS ON Area 20 sub-areas SPECIFICALLY
# 1. Summarize biosampling results and group as Nitinat, Non-Nitinat, and Unknown -----------------
area20STOCK.BDWR <- crestBDWR %>% 
  filter(AREA %in% c("20"), SAMPLE_TYPE=="Sport", SPECIES==124, DISPOSITION=="Kept", LEGAL_STATUS=="LEGAL", `(R) RESOLVED ORIGIN-STOCK ID`%notin%c("Unknown NA", "Unknown Unknown (<75% GSI assignment)")) %>% 
  group_by(YEAR, AREA, SUBAREA, MONTH, isSJ) %>% 
  summarize(n = n()) %>% 
  group_by(YEAR, MONTH, AREA, SUBAREA) %>%
  mutate(AreaMonthSampleTotal = sum(n),
         PropnMonthlySamplesSJ=n/AreaMonthSampleTotal)  

# 2. Join Biodata composition to catch and calculate # caught by ID -----------------
a20catchComp <- left_join(area20STOCK.BDWR,
                          catEst %>% 
                            rename(AREA=PFMA) %>% 
                            mutate(AREA = str_sub(AREA, start=6, end=9)) %>%
                            group_by(YEAR, AREA, CREEL_SUB_AREA, MONTH, DISPOSITION) %>% 
                            summarize(Est = sum(ESTIMATE),
                                      `%SE` = sum(PERCENT_STANDARD_ERROR)) %>% 
                            pivot_wider(names_from=DISPOSITION, values_from=c(Est, `%SE`)) %>%
                            select(YEAR, AREA, CREEL_SUB_AREA, MONTH, Est_Kept, `%SE_Kept`, `Est_Released Legal`, `%SE_Released Legal`, 
                                   `Est_Released Sub-legal`, `%SE_Released Sub-legal`) %>% 
                            rename(SUBAREA = CREEL_SUB_AREA),
                          by=c("AREA", "YEAR", "MONTH", "SUBAREA")) %>% 
  mutate(estKeptByID = PropnMonthlySamplesSJ*Est_Kept) 
```


```{r a20comp, out.width='1300px', out.height='500px', fig.align='left', fig.cap='Region-level ID of Chinook caught in Area 20. Data are expressed as a proportion of the number of biological samples collected each month. Results are from creel survey data and therefore only represent legal (<45cm, >80cm), kept catch. Gray shaded area indicates years with no GSI sampling, in which case Natural-origin stock ID is not possible. Red shading indicates 2023 results are incomplete.', eval=F}
...
ggplotly(
  ggplot() +
    geom_ribbon(data=area20REGION.BDWR %>%
    ...
    geom_point(data=area20REGION.BDWR%>%filter(AREA=="20"),
