...
For 2024 estimates used PL+D. No SK counted through the fence. No accounting needed for tribs as none observed in Harris/Lens.
...
```{r}
ggplot()  +
  geom_bar(data=sj.escHistorical %>% filter(`Waterbody Name`=="SAN JUAN RIVER", `Species Qualified`%in%c("SER","SEL")), 
...
ggplot()  +
  geom_bar(data=sj.escHistorical %>% 
             filter(`Waterbody Name`=="SAN JUAN RIVER", `Species Qualified`=="CM"), 
...
```{r}
mile4.fence %>% 
  filter(species=="pink", year==2024) %>%
  group_by(species, date) %>% 
  summarize(daily_total = sum(count))
```
...
For 2024 estimate used fence count of 5 adults for San Juan and PL+D for other systems. Little/no pinks expected in this system, especially in tributaries. *This is subject to change; see EscBatchHardCopy report for specific estimate abundance.*
