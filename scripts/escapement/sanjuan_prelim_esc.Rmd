...
# Raw SIL entry for lower/mid/upper designations compared to water level 
...
# Waterbody values:
wbdyVals <- readxl::read_excel(path=paste0(OD.dir, "WaterbodiesValues.xlsx")) %>% 
...
# AUC Data Table: 
AUCdataTable <- readxl::read_excel(path=paste0(OD.dir, "AUC_DataAnalysisTable_area20.xlsx"))

# sysNameCodes <- AUCdataTable %>% 
#   select(Section_AreaInspected WatershedCode) %>% 
#   group_by(Section_AreaInspected, WatershedCode) %>% 
#   summarize() %>% 
#   rename(systemName = Section_AreaInspected) %>%
#   print()


# Esc Estimates:
escEstSpp <- readxl::read_excel(path=paste0(OD.dir, "EscEstSppHeader_area20.xlsx")) %>% 
  left_join(.,
            wbdyVals,
...
Figure: Coho swim counts (raw adults) in the San Juan mainstem relative to cumulative fence counts (black line). 
...
For 2023 estimate we went with the expanded PL+D on Oct 4, even though there was potential for double-counting one school between the Oct 3/4 surveys (see above). Given that the majority of chinook were not observed to be holding at this time and trib swims occurred a couple of weeks later, trib counts should be subtracted from the total estimate. The PL+D estimate is 2642 adults + 67 jacks. Subtracting 24 expanded Harris and 21 expanded Lens Chinook results in 2597 adults + 67 jacks. *This is subject to change; see EscBatchHardCopy report for specific estimate abundance.*
...
ggplot()  +
  ...
  geom_bar(data=sj.esc %>% filter(`Waterbody Name`=="SAN JUAN RIVER", `Species Qualified`=="CO"), 
           aes(x=`Analysis Year`, y=`Unspecified Return`), stat="identity", colour="gray80", fill="gray80", alpha=0.8) +  
  # << MANUAL UPDATE y= >> 
  geom_bar(aes(x=analysis_year, y=2597), stat="identity", colour="orange", fill="orange", alpha=0.8) +   
  scale_x_continuous(breaks=seq(min(sj.esc$`Analysis Year`), analysis_year, by=2)) +
