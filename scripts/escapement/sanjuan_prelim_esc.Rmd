...
ggplot()  +
  ...
  geom_bar(data=escEstSpp %>% 
             ...
             select(Year, SystemName, AdultNaturalReco) %>% 
             summarize(n=sum(AdultNaturalReco)), 
             aes(x=analysis_year, y=n-24-21), stat="identity", colour="red", fill="dodger blue", alpha=0.8) +   scale_x_continuous(breaks=seq(min(sj.esc$`Analysis Year`), analysis_year, by=2)) +
